...
import { LeaveApprovalFormError, leaveApprovalFormProps } from '../../../../types/Leave';
import * as Yup from 'yup';
import { errorMessage } from '../../../../constants/app-hostel-constants';
import { ApproveLeaveFromParent, ApproveLeaveFromWarden } from '../../../../helpers/leave';
...
const FormSection = ({ ...props }: leaveApprovalFormProps) => {
  const [apiErrors, setApiErrors] = useState<ApiErrorResponse>();
  const [btnLoading, setBtnLoading] = useState<boolean>(false);
  const [errors, setErrors] = useState<LeaveApprovalFormError>();
    ...
    await validationSchema
      ...
      .then(() => {
        ...
        if (props?.role === 'Parent') {
          ApproveLeaveFromParent(data).then((result) => {
          ...
          ApproveLeaveFromWarden(data).then((result) => {
            if ((result && result.status === 'Success') || result.status === 200) {
            } else if (result && result.status === 400) {
              const formattedErrors: ApiErrorItem[] = result?.detail?.errors.map((errorItem: any) => {
                return {
