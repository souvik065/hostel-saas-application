...
import { CrewInputField, CrewIcon, CrewButton, CrewTypography } from '../../atoms';
import { faSquareMinus, faSquarePlus } from '@fortawesome/free-solid-svg-icons';
import { errorMessage } from '../../../constants/app-hostel-constants';
import './CrewMultipleInput.scss';
...
interface CrewMultipleInputProps<T extends Item> {
  items: T[];
...
const CrewMultipleInput = <T extends Item>({
  items,
  errors,
  onChange,
  onSetErrors,
  ...
  const handleAddInput = () => {
    const isAnyItemEmpty = updatedItems.some((item) => !(item[itemName] as string).trim());
    if (isAnyItemEmpty) {
        ...
        <Grid key={index} container>
          <Grid item xl={3} lg={4} md={4} sm={6} xs={12} px={1}>
            <CrewInputField
              ...
              placeholder={`${label}`}
              value={item[itemName]}
              onValueChange={(e) => handleInputFieldChange(e, index)}
              radius={8}
              fullWidth
...
export default CrewMultipleInput;
