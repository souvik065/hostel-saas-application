...
const API_URL = '/api/settings-leave';

export const getLeaveTypeDetailsByHostelId = async (id: string): Promise<any> => {
...
export const createLeaveType = async (toCreate: any): Promise<any> => {
  const session = await getSession();
...
export const updateLeaveType = async (toUpdate: any): Promise<any> => {
  const session = await getSession();
  const response = await fetchRetry(API_URL, {
    method: 'PUT',
    headers: {
...
export const deleteLeaveTypeDataById = async (id: string): Promise<any> => {
  const session = await getSession();
  const response = await fetchRetry(API_URL, {
    method: 'DELETE',
    headers: {
